...
knitr::include_graphics("images/folders.gif")
...
```{block, type="warning"}
Whatever you do, do NOT call the folder you store your R work in "R". You don't have to call it Data Skills, but if you call it simply R then R will have an existential crisis about saving into itself and you will find that you have problems reading and saving your files. 
```
...
* How many authors are there? `r fitb("7")`
* What is the name of the journal? `r fitb(c("Science", ignore_case = TRUE))`
* How many pages is the article? `r fitb(c("3"))` 

According to APA style:

* The title of the paper should be `r mcq(c("In bold text", "In italics", answer = "In normal text"))`.
* When using a direct in-text citation you should use `r mcq(c(answer = "and", "&"))` to connect the names of the authors.
* When citing a paper with multiple authors, if there are more than two authors the first time you cite the paper in the essay you should `r mcq(c(answer = "Write out all of the names", "Use et al."))`

`r hide("Explain this answer")`
```{r, echo = FALSE, results='asis'}
cat("*Et al* means *and others*. It's a common academic phrase that's used when referring to work with more than three authors.")
```
`r unhide()` 
...
```{block, type="danger"}
There is also a function called `read.csv()`. Be very careful NOT to use this function instead of `read_csv()` as they have different ways of naming columns. For the home, unless your results match ours **exactly** you will not get the marks which means you need to be careful to use the right functions.
```
