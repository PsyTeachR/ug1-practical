...
Use mutate to create a new column called `Happiness_Female` in `age_65max` which categorises participants based on whether they score above the median `ahiTotal` score for females. 

Then, group the data stored in  ```age_65max``` by `Happiness_Female`, and store it in an object named ```happy_female```. 

Finally, use summarise to calculate the median `cesdTotal` score for participants who scored above and below the median `ahiTotal` score for females and save it in a new object named `data_median_group`.

```{r T6}
age_65max <- mutate(age_65max, Happiness_Female = (ahiTotal > 74))
happy_female <- group_by(age_65max, Happiness_Female)

data_median_group <- summarise(happy_female, median_score = median(cesdTotal))
...
* How would you change the code to group by education rather than `Happiness_Female`?
